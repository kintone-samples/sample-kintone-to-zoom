...
((PLUGIN_ID) => {
  ...
  // 「ホスト」「参加」ボタンをレコード詳細画面に表示し、zoomミーティング画面表示用のiframeを用意する処理
  const detailHandle = event => {
    ...
    // スペースフィールド「zoom」にzoom画面表示用のiframeを配置する処理
  ...
  // レコード詳細画面が表示されたときに、「ホスト」「参加」ボタンを表示し、zoom画面表示用のiframeを用意する処理
